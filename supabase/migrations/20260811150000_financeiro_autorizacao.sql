-- Autorizacao financeira real (FIN-0).
-- Substitui as permissoes fixas da UI por permissao resolvida no banco.
-- Aditiva: nenhuma coluna/funcao existente e removida.

-- Resolve se o usuario atual possui uma permissao do catalogo de perfis_acesso.
-- Papel 'admin' e 'novus_owner' passam sempre.
--
-- Compatibilidade: 'financeiro.liquidar' e 'financeiro.cancelamento' sao codigos novos.
-- Os tres perfis de sistema (ADMINISTRADOR, OPERADOR, CONSULTA) sao imutaveis por trigger
-- (protect_perfis_sistema), entao nao ha backfill de dados: a funcao aceita o codigo
-- analogo ja concedido. Quem lanca no financeiro (financeiro.create) pode liquidar; quem
-- estorna (financeiro.estorno) pode cancelar. Perfis novos criados pelo administrador
-- podem conceder os codigos novos diretamente, sem depender desse fallback.
...
-- Versao que levanta excecao; usada dentro das RPCs financeiras.
CREATE OR REPLACE FUNCTION public.financeiro_exigir_permissao(p_acao text)
RETURNS void
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
AS $$
BEGIN
  IF NOT financeiro_pode(p_acao) THEN
    RAISE EXCEPTION 'Permissao negada: %', p_acao USING ERRCODE = '42501';
  END IF;
END;
$$;

-- Retorna o mapa de capacidades da tela de movimentacoes numa unica chamada.
...
GRANT EXECUTE ON FUNCTION public.financeiro_pode(text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.financeiro_permissoes() TO authenticated;

-- financeiro_cancelar_titulo: guard de autorizacao adicionado; restante do corpo inalterado.
CREATE OR REPLACE FUNCTION public.financeiro_cancelar_titulo(p_titulo_id uuid, p_tipo_titulo text, p_motivo text, p_idempotency_key uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
-- financeiro_estornar_liquidacao: guard de autorizacao adicionado; restante do corpo inalterado.
CREATE OR REPLACE FUNCTION public.financeiro_estornar_liquidacao(p_liquidacao_id uuid, p_motivo text, p_idempotency_key uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
-- financeiro_liquidar_titulo: guard de autorizacao adicionado; restante do corpo inalterado.
CREATE OR REPLACE FUNCTION public.financeiro_liquidar_titulo(p_titulo_id uuid, p_tipo_titulo text, p_valor numeric, p_data_pagamento date, p_forma_pagamento text, p_idempotency_key uuid, p_conta_bancaria_id uuid DEFAULT NULL::uuid, p_observacoes text DEFAULT NULL::text, p_multi_baixa jsonb DEFAULT '[]'::jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
