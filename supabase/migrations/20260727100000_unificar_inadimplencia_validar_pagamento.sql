...
-- ============================================================
-- Unificação parcial (mínima) dos dois mecanismos de crédito
-- (docs/CONTRATOS_CANONICOS_ERP.md §6): validar_pagamento_venda
-- (usado por converter_orcamento_em_venda) nunca checava títulos
-- vencidos em contas_receber — só verificar_autorizacao_venda
-- (Porta 3, usado por VendaFormModal) fazia essa checagem. Um
-- cliente inadimplente passava batido na conversão de orçamento
-- em venda a prazo. Fecha só esse gap: reaproveita
-- verificar_autorizacao_venda (já testado) para importar
-- exclusivamente o bloqueio CLIENTE_INADIMPLENTE — não duplica
-- CLIENTE_BLOQUEADO/LIMITE_CREDIARIO_EXCEDIDO (já cobertos por
-- CLIENTE_BLOQUEADO/CREDIARIO_SEM_LIMITE existentes) nem altera o
-- aviso CLIENTE_EM_ANALISE (fora do escopo mínimo decidido).
-- ============================================================
...
DECLARE
  ...
  v_credito         jsonb;
  v_b               jsonb;
  v_inadimplencia_add boolean := false;
BEGIN
  ...
  -- Reaproveita a checagem de títulos vencidos já validada em
  -- verificar_autorizacao_venda (Porta 3) em vez de duplicar a
  -- query em contas_receber aqui.
  v_credito := public.verificar_autorizacao_venda(v_venda.cliente_id, v_venda.empresa_representada_id, v_venda.valor_total);
  ...
  LOOP
    ...
    IF v_credi_natureza IS NOT NULL AND v_lp.natureza_id = v_credi_natureza THEN
      ...
      IF NOT v_inadimplencia_add THEN
        FOR v_b IN SELECT * FROM jsonb_array_elements(COALESCE(v_credito->'bloqueios','[]'::jsonb))
        LOOP
          IF v_b->>'codigo' = 'CLIENTE_INADIMPLENTE' THEN
            v_erros := v_erros || jsonb_build_object(
              'codigo','CLIENTE_INADIMPLENTE','categoria','POLITICA',
              'mensagem', v_b->>'motivo'
            );
          END IF;
        END LOOP;
        v_inadimplencia_add := true;
      END IF;
    END IF;
