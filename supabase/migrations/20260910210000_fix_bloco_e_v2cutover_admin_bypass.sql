-- Bloco E: cutover de versão de assinatura de webhook (v1/dual/v2), por
-- tenant. Padrão diferente dos blocos anteriores: aqui não havia NENHUM
-- fallback de empresa (nem user_has_access_to_empresa) — só
-- has_role(auth.uid(),'admin') puro, GLOBAL, apesar da função receber
-- p_tenant explícito. Qualquer admin de qualquer empresa podia promover/
-- rebaixar a versão de assinatura de webhook de OUTRA empresa
-- (check_v2_readiness/promote_to_dual/promote_to_v2_only/rollback_to_dual/
-- rollback_to_v1), o que pode quebrar ou enfraquecer a verificação HMAC de
-- um satélite alheio. Fix: has_role_for_empresa(admin, p_tenant) — mesmo
-- bypass novus_owner já formalizado, agora exigindo vínculo real com o
-- tenant alvo pra qualquer outro admin.
--
-- precheck_source_system_nome_consistency também corrigida aqui: por
-- desenho ela agrega dado de TODAS as empresas (sem parâmetro de tenant),
-- então "qualquer admin" não é o público certo — é ferramenta de operação
-- de sistema, mesmo perfil de listar_satelites_disponiveis/
-- criar_responsavel_centelha (já novus_owner-only). Trocado 'admin' por
-- 'novus_owner'.

CREATE OR REPLACE FUNCTION public.check_v2_readiness(p_tenant uuid, p_nome text, p_min_events integer DEFAULT 20)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
DECLARE
  ...
  v_reason text := 'ok';
BEGIN
  IF NOT public.has_role_for_empresa(auth.uid(), 'admin', p_tenant) THEN
...
CREATE OR REPLACE FUNCTION public.promote_to_v2_only(p_tenant uuid, p_nome text, p_min_events integer DEFAULT 20)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.precheck_source_system_nome_consistency()
 RETURNS TABLE(empresa_representada_id uuid, nome text, signature_version text, distinct_source_systems text[], event_count bigint, consistent boolean)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
