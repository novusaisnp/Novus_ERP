...
export const SidebarUserFooter: React.FC<SidebarUserFooterProps> = ({
  ...
  return (
    <div className="absolute bottom-0 left-0 right-0 border-t border-blue-600 bg-blue-800" style={{ height: '60px' }}>
      <div className="h-full p-2">
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <button
              className={`w-full flex items-center text-white hover:bg-blue-700 transition-colors p-2 rounded h-full ${
              ...
              {isHovered && (
                <>
                  <div className="ml-3 text-left flex-1 min-w-0 overflow-hidden">
                    <div className="text-sm font-medium text-white whitespace-nowrap overflow-hidden text-ellipsis" style={{ maxWidth: '160px' }}>
                      {userName}
                    </div>
                    <div className="text-xs text-blue-200 whitespace-nowrap overflow-hidden text-ellipsis" style={{ maxWidth: '160px' }}>
                      {userEmail}
                    </div>
                  </div>
