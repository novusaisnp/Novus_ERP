...
**Concorrência real com duas conexões: provada em 2026-09-13** (ver `STATUS.md`) — `scripts/check-etapa1-concorrencia.mjs` cria fixture commitada, dispara 2 inserções verdadeiramente concorrentes (processos separados) na mesma conta bancária e no mesmo produto contra o banco real vinculado, confirma resultado sem lost-update (saldo soma as 2 movimentações, estoque desconta as 2 saídas), e limpa a fixture por completo (0 resíduo verificado). Achou e corrigiu de passagem 3 bugs reais de FK/trigger em tabelas de histórico pré-existentes (hard-delete de `movimentacoes_bancarias` sempre falhava; ciclo de FK `empresas_representadas`↔`plano_contas`; recriação de histórico de estoque no próprio delete) — nenhum é da Etapa 1, documentados no script. **Ainda não testado**: E2E e os perfis restritos nos fluxos financeiros/compras afetados (fluxos de negócio completos com dois clientes simultâneos).
