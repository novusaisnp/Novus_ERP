...
**Caso 2 — Crédito/inadimplência (a implementar):** `verificar_autorizacao_venda(cliente_id, empresa_id, valor_pretendido)`, cruzando `ClientePoliticaPagamento` (status, limite_crediario/utilizado, dias_max_atraso) com títulos em `contas_receber` vencidos e não conciliados. Se bloqueado e `pode_ser_superado = true`, o satélite oferece a opção de exceção; a autorização em si passa por `autorizacaoOverrideSchema` (`bloqueio_codigo`, `usuario_id`, `justificativa`) — **o NOVUS reconfirma a permissão do usuário no servidor** (nunca confia na alegação do satélite) e registra a exceção via auditoria (mesmo padrão de `get_audit_trail`, já usado em Gestão Bancária).
...
4. **Fase 3:** implementar `verificar_autorizacao_venda` (Porta 3, crédito/inadimplência, §6) com override auditado; formalizar `usuarioCanonicalSchema`; estender cobertura às tabelas restantes (ContasPagar, RH/Colaboradores).
