...
## 11. Dois cenários de uso e pilares técnicos para maturidade de integração

Registrado pelo usuário em 2026-07-27 (documento fonte: `docs/ORIENTAÇÃO GERAL SOBRE DESENVOLVIMENTO DO ERP.MD`, absorvido aqui — não é conteúdo novo em relação à visão do §1, mas explicita o que falta para o hub estar tecnicamente maduro para integração, não só desenhado para ela).

### 11.1 Os dois cenários que o NOVUS precisa atender ao mesmo tempo

- **Cenário 1 — Operação direta (pequeno negócio):** lojinha, oficina, conveniência. O NOVUS é a **única** ferramenta de gestão — vendas, estoque, financeiro e fiscal direto na interface. Prioridade: usabilidade e simplicidade (é a lente usada na avaliação de mercado do módulo de Vendas, seção acima nesta sessão em `docs/STATUS.md`).
- **Cenário 2 — Pseudo-backend para satélites** (Novus Educacional e outros futuros): o NOVUS é o "cérebro" financeiro/de recursos, processando transações geradas por sistemas especializados externos. Prioridade: capacidade de integração, flexibilidade, segurança e resiliência.

O mesmo núcleo (Cliente/Venda/Contrato/Título/Estoque, §5) atende aos dois — a diferença é só **quem opera a UI diretamente** vs. **quem envia dados via as três portas** (§2). Nenhuma tabela ou fluxo deve ser exclusivo de um cenário.

### 11.2 Pilares técnicos — estado real de cada um (não aspiracional, checado contra o código)

| Pilar | Estado | Onde / o que falta |
|---|---|---|
| APIs REST para ingestão | ✅ existe | Edge functions (`sync-webhook`, `data-validator`) + RPCs SECURITY DEFINER — é o padrão já usado em todo o projeto, não Express/Fastify tradicional. |
| Webhooks **de saída** (NOVUS → satélite) | 🟡 parcial | `Webhooks.tsx`/`WebhookConfigModal` já configuram integrações de saída por empresa (§3), mas não há um catálogo formal de eventos (`novo_cliente_cadastrado`, `estoque_baixo_produto_X`, `pagamento_recebido_contrato_Y`) nem disparo automático desses eventos hoje — é infraestrutura de configuração sem os gatilhos de negócio ainda pendurados nela. |
| Idempotência | ✅ existe | Padrão `idempotency_key`/`hash_payload` já formalizado no envelope (§4) e implementado de verdade em `gerar_contas_receber_da_venda`, `converter_orcamento_em_venda`, `criarPagamentoComParcelas`. Não é aspiracional, é o padrão real do projeto. |
| Permissões granulares / IAM interno | ✅ existe | `has_role` (papéis) + `has_permissao` (permissão granular via `perfis_acesso.permissoes`, criado na Porta 3 desta sessão) + RLS por `empresa_representada_id` em toda tabela. Cobre bem o Cenário 1 e chamadas internas. |
| IAM para parceiros externos (OAuth2/OIDC) | ❌ não existe | Hoje a autenticação de satélite é via HMAC por empresa (`sync-webhook`), não um provedor de identidade padrão de mercado. Suficiente para o volume atual; se um satélite real precisar de delegação de acesso mais fina (ex.: um usuário do satélite agindo "como" um usuário específico do NOVUS), HMAC por empresa não resolve — decisão a tomar quando houver um satélite real, não antes (mesmo princípio já registrado: não abstrair sem caso real). |
| Modelo de dados flexível (custom fields) | ❌ não existe | Nenhuma entidade core (Cliente, Produto, Venda, Contrato) aceita campo customizado por tenant hoje — é schema fixo. Adicionar exigiria decidir entre coluna `jsonb` genérica (`metadata`) por tabela vs. tabela EAV separada; **decisão de arquitetura a tomar com o usuário antes de começar**, não uma migração pequena. |
| Motor de regras de negócio configurável | 🟡 embrionário | O padrão mais próximo já existente é "Regras de Classificação de Receita" (citado em §10 Fase 2 como base a reaproveitar para adaptadores por `source_system`) — mas não há hoje um motor genérico para alçada de aprovação, política de preço ou comissionamento. Comissionamento em particular está diretamente ligado ao gap "sem vendedor/operador na venda" já identificado na avaliação de mercado desta sessão (`docs/STATUS.md`) — **o mesmo achado, visto por dois ângulos diferentes** (mercado E arquitetura de integração apontam pro mesmo buraco). |
| Logging/auditoria | ✅ existe (interno) | `historico_*` (Estoque, Gestão Bancária, agora Porta 3) + `get_audit_trail`. Cobre auditoria de ação de usuário; não cobre ainda monitoramento de saúde por integração de satélite além do que `SyncDashboard` já mede por webhook (§10 Fase 5, ainda não feita). |
| Retentativa automática (retry) | ❌ não existe | Falha de entrega de webhook de saída hoje não tem retry automático documentado — candidato a fechar junto com o catálogo de eventos acima. |
| Filas de mensagens (RabbitMQ/Kafka) | ❌ não existe, e é uma decisão grande | Todo o projeto roda hoje em cima de Supabase (Postgres + Edge Functions serverless) — adotar uma fila de mensagens dedicada é uma peça de infraestrutura nova, não uma biblioteca a mais. **Não iniciar sem decisão explícita do usuário**: mesmo raciocínio já registrado sobre dependência de fornecedor único (`docs/STATUS.md`, seção de riscos arquiteturais) — avaliar se o volume real de algum satélite futuro justifica antes de comprometer a stack com uma peça nova. |
| CRM básico (histórico/fidelidade de cliente) | ❌ não existe | Mesmo gap apontado na avaliação de mercado de Vendas desta sessão: tela de venda não mostra histórico de compras/ticket médio do cliente. Cliente já é "fonte única da verdade" (§5) — falta só a consulta agregada, não mudança de schema. |
| Estoque sincronizado em tempo real | ✅ em boa forma | `validar_saldo_estoque` (Porta 3) + `baixar_estoque_venda`/`estornar_estoque_venda` (ativados nesta sessão, ver `docs/STATUS.md`) + sincronismo `produtos.estoque_atual` ↔ `estoque_saldos` (corrigido nesta sessão). Esse pilar específico saiu de "não implementado" para "funcional" nesta mesma sessão. |
| Fiscal a partir de qualquer origem | ✅ existe | Já coberto em §5 — fiscal é sempre gerado pelo NOVUS a partir de uma Venda já existente, não importa se a Venda veio de UI direta ou satélite. |
| Financeiro integrado (toda venda alimenta Contas a Receber/Fluxo de Caixa) | ✅ existe | É a arquitetura de Portas 1/2 já documentada (§2) — não depende de origem. |

### 11.3 Leitura prática

A maior parte dos pilares "faltantes" acima não são lacunas desconhecidas — são decisões de infraestrutura (fila de mensagens, OAuth2 externo, custom fields) que exigem alinhamento explícito com o usuário antes de começar, não itens para atacar silenciosamente numa sessão. Os dois pilares mais "baratos" de avançar sem grande decisão de arquitetura são: catálogo de eventos de webhook de saída (infraestrutura de configuração já existe, falta só os gatilhos) e motor de comissionamento básico (que já tem prioridade própria como item de backlog da avaliação de mercado de Vendas, por resolver dois problemas com uma peça só: comissionamento E vendedor/operador na venda).
